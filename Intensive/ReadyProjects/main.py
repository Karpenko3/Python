...
money_count = 1500
axe_level = 1
workers_speed = 200       # Нова змінна: початкова затримка робітників
workers_capacity = 5      # Нова змінна: скільки дерева несе робітник
wood_cost = 0.5           # Нова змінна: ціна за 1 дерево

workers_inventory = []
workers_labels = []

# --- Функції покращень (з модального вікна) ---
...
def add_worker():
    worker_label = Label(root, text="👷", font=("Arial", 25))
    worker_label.pack()
...
def update_game():
    ...
    for i in range(workers_count):
        ...
        if collision(workers_labels[i], factory) and workers_inventory[i] == 1:
            ...
            set_money(workers_capacity * wood_cost) # Прибуток тепер залежить від місткості та ціни дерева

    update_stats()
    root.after(workers_speed, update_game) # Швидкість тепер залежить від змінної workers_speed
...
def sell_wood(obj):
    if collision(factory, obj):
        set_money(wood_count * wood_cost) # Прибуток від продажу залежить від ціни дерева
        set_wood(-wood_count)
...
# --- Налаштування вікна та UI ---
root = Tk()
root.geometry('600x300')
root.title('WoodChopper')
root.resizable(False, False)

# Нове пакування для верхнього меню
